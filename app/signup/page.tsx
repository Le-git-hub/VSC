...
export default function Signup() {
  ...
  const [signupResult, setSignupResult] = useState<null | boolean>(null);
  ...
  const handleClick = async (username: string, password: string) => {
    if (username.length < 1 || password.length < 8) {
        ...
        setErrorSuccessMessage("Password must contain at least 8 characters");
        return;
    } 
    try {
      const usernameRes = await fetch("http://localhost:5000/api/check_username", {
        method: "POST",
        headers: {
      ...
      if (usernameRes.status != 200) {
        setSignupResult(false);
        setErrorSuccessMessage("Username not available");
        return;
      }
      ...
      if (res_json.success === true) {
        ...
        router.push("/")
        setSignupResult(true)
        setErrorSuccessMessage("Signup Succesful")
      } else {
        setSignupResult(false);
        setErrorSuccessMessage("Signup failed, "+res_json.message);
      }
    } catch (error) {
      setErrorSuccessMessage("Error Occured, "+error);
      setSignupResult(false);
    }
  };
  ...
  return <div className="text-2xl font-bold flex items-center justify-center h-screen w-screen bg-linear-to-r from-cyan-200 to-blue-800 drop-shadow-2xl">
    <div className="bg-[#252627] w-xl h-[600px] rounded-2xl flex flex-col items-center">
        ...
        <h1 className="text-gray-200 text-3xl font-bold mt-5">Signup</h1>
        <h2 className="text-gray-200 text-lg font-light mt-4 text-center w-[80%]">Create a new account, only requiring username and password</h2>
        <div className="flex flex-col items-center justify-center mt-[15%] space-y-2 w-[90%]">
            ...
            <div className="relative w-full">
              <input type={eyeClick? "password" : "text"} placeholder="Password" maxLength={32} minLength={8} className="w-full h-15 rounded-md border-2 border-gray-300 p-2" value={password}
            ...
            <button className="w-full h-15 rounded-md bg-blue-400 text-gray-700 shadow-lg hover:bg-blue-500 transition-all duration-200" onClick={() => handleClick(username, password)}>Signup</button>
            {signupResult === true ? (
            ...
            <h2 className="text-gray-200 font-light text-lg mt-2">
              Already have an account? <Link href="/login" className="text-blue-400 cursor-pointer">Login</Link>
            </h2>
        </div>
    </div>
